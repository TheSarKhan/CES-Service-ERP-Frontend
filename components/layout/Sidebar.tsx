...
import { useStockAlertSummary, useWarrantySummary } from '@/hooks/use-inventory';
...
export function Sidebar() {
  ...
  // Products below their threshold, on the warehouse entry itself — that is where you go to act.
  const { data: stockAlerts } = useStockAlertSummary();
  const lowStockCount = stockAlerts?.total ?? 0;
                          ...
                          {childrenVisible.map((child) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {child.key === 'M19_WAREHOUSE_MAIN' && lowStockCount > 0 && (
                                  <span className="sd-badge">{lowStockCount}</span>
                                )}
